...
# 4. Load sample data (restaurants, menus, test users)
make seed
```
...
SPRING_KAFKA_BOOTSTRAP_SERVERS: kafka-0.kafka.svc:9092
```
...
type Config struct {
    ...
    KafkaBrokers   string `env:"KAFKA_BROKERS" envDefault:"kafka-0.kafka.svc:9092"`
...
export const config = {
  ...
  kafkaBrokers: (process.env.KAFKA_BROKERS || 'kafka-0.kafka.svc:9092').split(','),
...
| Kafka consumer not receiving | Check consumer group: `kubectl exec kafka-0 -n kafka -- kafka-consumer-groups.sh --list` |
